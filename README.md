...
| 기능 | 설명 |
|------|------|
| **규칙 기반 자동 탐지** | 9개 대분류 · 25개 세부 규칙으로 항목 간 불일치, 급변동, 미입력 패턴 등을 자동 탐지합니다 |
| **검토 우선도 지수** | 탐지 결과를 학교별 0~100 점수로 환산하여 검토 순서를 제시합니다 (V·C·R 3요소 가중평균) |
| **4단계 심각도 분류** | 즉시 검토 · 우선 검토 대상 · 일반 검토 · 참고 — 4단계로 분류합니다 |
| **AI 보조 분석** | Gemini 기반 자연어 대화로 탐지 항목에 대한 맥락 설명과 추가 분석을 제공합니다 |
| **유사학교 비교** | 같은 자치구 내 학교 간 횡단 비교로 이상 편차를 탐지합니다 |
| **시계열 추세 감지** | 3개년 데이터에서 단조 추세, 급변동, 동일값 반복 등을 식별합니다 |
...
<p align="center">
  <a href="https://edudata-watch-production.up.railway.app/">
    <img src="assets/screenshot-dashboard.png" width="750" alt="대시보드"/>
  </a>
</p>

**공시 데이터 검토 우선 후보** — 검토 우선도 지수 상위 학교를 카드와 테이블로 확인할 수 있습니다. 심각도 · 대분류 · 지역별 필터를 지원합니다.
...
| 대분류 | 영역 | 규칙 수 | 설명 |
|--------|------|---------|------|
| **C1** | 항목 간 불균형 | 7개 | 학생수 · 학급수 · 교원수 간 비율 불일치 탐지 |
| **C2** | 학생 · 재정 연동 | 3개 | 학생수 안정인데 급식비 · 회계가 급변동하는 패턴 |
| **C3** | 학생 안전 | 2개 | 학교폭력 피해 대비 보호조치 · 선도교육 미조치 |
| **B1** | 전년 대비 급변동 | 4개 | 학생 · 교원 · 회계 · 진학률의 전년 대비 급격한 변동 |
| **D2** | 유사학교 비교 | 2개 | 같은 자치구 내 학교 간 극단적 편차 (IQR · 백분위) |
| **E** | 누락 · 미갱신 | 3개 | 3년 연속 미입력 · 단독 미입력 · 동일값 반복 |
| **C5** | 학년 진급 | 1개 | 진급 시 학생 이탈률 비대칭 탐지 |
| **F1'** | 교차 검증 | 1개 | 학교알리미 vs KESS 교원수 불일치 |
| **G1** | 장기 추세 | 2개 | 다년 단조 추세 및 추세 급변동 |

<details>
<summary><b>25개 세부 규칙 전체 보기</b></summary>
<br/>

| 규칙 ID | 규칙명 | 설명 |
|---------|--------|------|
| C1-1 | 학생수 대비 학급수 불균형 | 학급당 학생수가 유사학교 평균 대비 ±1.5σ 이상 |
| C1-2 | 학생수 대비 교원수 불균형 | 교원 1인당 학생수가 유사학교 대비 극단적 |
| C1-3 | 학급수 대비 교원수 불균형 | 학급당 교원수 비율 이상 |
| C1-4 | 학생 증감과 학급 방향 불일치 | 학생수 변동과 학급수 변동의 방향이 반대 |
| C1-5 | 학생 증감과 교원 방향 불일치 | 학생수 변동과 교원수 변동의 방향이 반대 |
| C1-6 | 학급 증감과 교원 방향 불일치 | 학급수 변동과 교원수 변동의 방향이 반대 |
| C1-7 | 학생수 대비 학급수 (KESS) | KESS 데이터 기준 학급당 학생수 검증 |
| C2-1 | 학생 안정 · 급식비 변동 | 학생수 ±5% 이내인데 급식비 ±10% 이상 변동 |
| C2-2 | 학생 안정 · 학교회계 변동 | 학생수 ±5% 이내인데 학교회계 ±10% 이상 변동 |
| C2-3 | 학생 안정 · 급식비 강한 변동 | 학생수 ±5% 이내인데 급식비 ±30% 이상 변동 |
| C3-3 | 미조치 피해 | 피해학생 존재하는데 보호조치 0건 |
| C3-4 | 학교폭력 급증 | 학교폭력 관련 수치의 전년 대비 급격한 증가 |
| B1-1 | 학생수 급변동 | 전년 대비 학생수 ±10% 이상 변동 |
| B1-2 | 학급수 급변동 | 전년 대비 학급수 ±10% 이상 변동 |
| B1-3 | 교원수 급변동 | 전년 대비 교원수 ±10% 이상 변동 |
| B1-4 | 학교회계 급변동 | 전년 대비 학교회계 ±50% 이상 변동 |
| D2-1 | 유사학교 대비 극단값 | 같은 자치구 내 IQR 기준 극단값 탐지 |
| D2-2 | 유사학교 대비 백분위 이상 | 같은 자치구 내 상하위 10% 백분위 탐지 |
| E1-1 | 3년 연속 미입력 | 동일 항목 3개년 연속 미입력 |
| E1-2 | 단독 미입력 | 유사학교 대부분이 입력한 항목을 단독 미입력 |
| E2-2 | 3년 동일값 반복 | 3개년 동안 동일한 값이 반복 입력 |
| C5-1 | 진급 시 학생 이탈 | 학년 진급 시 학생수 감소율이 비대칭 임계 초과 |
| F1' | 교원수 교차 불일치 | 학교알리미와 KESS 교원수 3명 이상 차이 |
| G1-1 | 다년 단조 추세 | 3개년 회귀 기울기 기반 지속적 증감 추세 |
| G1-2 | 추세 급변동 | 기존 추세선 대비 급격한 이탈 |
