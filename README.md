...
| **검토 우선도 지수** | 탐지 결과를 학교별 0~100 점수로 환산하여 검토 순서를 제시합니다 (V=신호 강도 · C=탐지 영역 수 · R=반복 여부) |
...
| 대분류 | 영역 | 규칙 수 | 설명 |
|--------|------|---------|------|
| **C1** | 학생·자원 연동 점검 | 6개 | 학생수 · 학급수 · 교원수 간 비율 불일치 및 방향 불일치 탐지 |
| **C3** | 미조치 피해 점검 | 2개 | 학교폭력 피해 발생에도 보호조치가 이루어지지 않은 패턴 |
| **B1** | 전년 대비 급변동 | 6개 | 학생 · 교원 · 회계 · 진학률 · 학폭의 전년 대비 급격한 변동 |
| **D2** | 유사학교 대비 편차 | 2개 | 같은 자치구 내 학교 간 극단적 편차 (IQR · 백분위) |
| **C2** | 학생·재정 연동 점검 | 2개 | 학생수 안정인데 급식비가 급변동하는 패턴 |
| **E** | 누락·미갱신 점검 | 3개 | 3년 연속 미입력 · 단독 미입력 · 동일값 반복 |
| **C5** | 학년 진급 인원 점검 | 1개 | 진급 시 학생 이탈률 비대칭 탐지 |
| **F1'** | 연계 시점 차이 점검 | 1개 | 학교알리미 vs KESS 교원수 불일치 |
| **G1** | 시계열 추세 점검 | 2개 | 다년 단조 추세 및 추세 급변동 |

<details>
<summary><b>C1. 학생·자원 연동 점검 (6개)</b> — 학생수 변화와 자원 배치(학급, 교원) 변화의 방향이 어긋나거나 비율이 크게 깨진 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>C3. 미조치 피해 점검 (2개)</b> — 학교폭력 심의에서 피해학생이 존재하는데 보호조치 건수가 0인 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>B1. 전년 대비 급변동 (6개)</b> — 한 학교의 한 항목이 전년 대비 비정상적으로 큰 변동을 보이는 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>D2. 유사학교 대비 편차 (2개)</b> — 같은 자치구 내 유사학교와 비교했을 때 한 학교만 두드러지게 다른 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>C2. 학생·재정 연동 점검 (2개)</b> — 학생수가 안정적인데 재정 항목이 급변동하는 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>E. 누락·미갱신 점검 (3개)</b> — 특정 항목이 여러 연도에 걸쳐 누락되거나 동일한 값이 반복되는 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>C5. 학년 진급 인원 점검 (1개)</b> — 학년 진급 시 자연 감소 범위를 초과하는 인원 변화를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>F1'. 연계 시점 차이 점검 (1개)</b> — 서로 다른 공시 출처 간 동일 항목의 값이 다른 경우를 탐지합니다.</summary>
<br/>
...
<details>
<summary><b>G1. 시계열 추세 점검 (2개)</b> — 3개년 데이터에서 지속적인 추세나 추세의 급격한 변화를 탐지합니다.</summary>
<br/>
